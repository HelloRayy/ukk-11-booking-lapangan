// PERAN FILE: Layar Pembayaran QRIS Dinamis Terintegrasi di Dalam Right Panel (Tahap 2)
      ...
      <div>
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-[#8e8e8e] mt-0.5">
            Pindai kode QR untuk menyelesaikan reservasi
          </p>
        </div>

        {/* 3. Kartu Barcode QRIS */}
        <div className="p-4 rounded-[14px] bg-white text-black mb-3.5 shadow-xl flex flex-col items-center">
          {/* Header Logo QRIS */}
          ...
          <div className="w-44 h-44 bg-white p-1 rounded-[8px] flex items-center justify-center">
            ...
            >
              ...
              <rect x="42" y="20" width="6" height="6" fill="black" />
              <rect x="52" y="20" width="6" height="6" fill="black" />
              <rect x="72" y="20" width="6" height="6" fill="black" />
              <rect x="82" y="42" width="6" height="6" fill="black" />
              <rect x="82" y="52" width="6" height="6" fill="black" />
            </svg>
          </div>
        ...
        <div className="p-3.5 rounded-[12px] bg-[#222222] border border-[#2e2e2e] mb-3">
          <div className="flex items-center justify-between text-xs text-[#8e8e8e] mb-1">
            <span>Tagihan Pembayaran ({paymentType === 'dp' ? 'DP 50%' : 'Lunas 100%'})</span>
            <span className="text-white font-medium">{selectedSlot.courtName}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-[#8e8e8e]">Total Transfer</span>
          ...
          {paymentType === 'dp' && (
            <div className="mt-2 pt-2 border-t border-white/5 text-[11px] text-[#8e8e8e]">
              Sisa pelunasan <span className="text-white font-semibold">Rp {remainingAmount.toLocaleString('id-ID')}</span> dibayarkan saat tiba di kasir arena.
            </div>
          )}
        </div>

        {/* 5. Petunjuk Singkat Pembayaran */}
        <div className="p-3 rounded-[10px] bg-white/[0.03] border border-white/5 text-[11px] text-[#8e8e8e] space-y-1">
          <div className="font-semibold text-white mb-1">Cara Pembayaran:</div>
          <div>1. Buka aplikasi m-Banking atau e-Wallet (BCA, GoPay, OVO, Dana).</div>
          <div>2. Pilih menu Scan / Bayar QRIS dan arahkan kamera ke barcode di atas.</div>
          <div>3. Periksa nama merchant dan selesaikan transaksi.</div>
        </div>
      </div>
